...
<title>{{ shop_name }} - {{ '_' | jtext: 'FILES_JKTHEME_CASSIOPEIA_EMAIL_AFFILIATE_CHANGES_ACCOUNT' }}</title>
<style>
						...
						<tbody>
											...
											<td style="padding: 7px 0">
												<font size="2" face="Open-sans, sans-serif" color="#555454">
													<p style="border-bottom: 1px solid #D6D4D4; margin: 3px 0 7px; text-transform: uppercase; font-weight: 500; font-size: 18px; padding-bottom: 10px" data-html-only="1">
														...
														{% if transaction %}
															{{ 'sprintf' | jtext: 'FILES_JKTHEME_CASSIOPEIA_EMAIL_AFFILIATE_TRANSACTION_1', sum_cy }}
														{% else %}
															{{ 'sprintf' | jtext: 'FILES_JKTHEME_CASSIOPEIA_EMAIL_AFFILIATE_TRANSACTION_0', sum_cy }}
														{% endif %}
													</p>
													<span style="color:#777">
														{{ '_' | jtext: 'FILES_JKTHEME_CASSIOPEIA_EMAIL_AFFILIATE_ON_YOUR_ACCOUNT' }}:
														<strong style="color:#333">
															{% if user_cash %}
																{% for cash in user_cash %}
																	{{ cash.sum }}{{ cash.currency }}
																{% endfor %}
															{% else %}
																0
															{% endif %}
														...
														<br/>
														{{ '_' | jtext: 'FILES_JKTHEME_CASSIOPEIA_EMAIL_AFFILIATE_AVAILABLE_FOR_OUTPUT' }}:
														<strong style="color:#333">
															{% if cash_available %}
							...
							<tr>
								<td class="linkbelow" style="border-top: 1px solid #e2e7eb; padding: 7px 0">
									<font size="2" face="Open-sans, sans-serif" color="#555454">
										<em>* {{ '_' | jtext: 'FILES_JKTHEME_CASSIOPEIA_EMAIL_FOOTER' }}</em> 
									</font>
								</td>
							</tr>
							<tr>
							...
							<tr>
								<td class="footer" style="border-top: 4px solid #5e6981; padding:7px 0">
									<span>
										<a href="{{ shop_url }}" style="color: #5e6981">{{ shop_name }}</a>
									</span>
								</td>
							</tr>
						</tbody>
